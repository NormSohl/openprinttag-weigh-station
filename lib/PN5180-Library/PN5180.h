...
// VENDORED: bound every spin-wait in this library (BUSY-line, getIRQStatus(), and
// the ISO15693 RX wait). The stock loops never time out and hold the SPI bus while
// they spin, so a hung reader freezes displayTask, which shares the bus. Normal
// waits complete in well under a millisecond; 250 ms is far beyond any real op.
// See CLAUDE.md (the PN5180-hang-freezes-display quirk).
#define PN5180_BUSY_TIMEOUT_MS 250
