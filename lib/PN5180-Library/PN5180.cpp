...
bool PN5180::setRF_on() {
  ...
  // bounded (vendored): don't hold the bus forever if the field never reports up
  { uint32_t _t0 = millis();
    while (0 == (TX_RFON_IRQ_STAT & getIRQStatus()))
      if (millis() - _t0 > PN5180_BUSY_TIMEOUT_MS) break; }
...
bool PN5180::setRF_off() {
  ...
  // bounded (vendored): don't hold the bus forever if the field never reports off
  { uint32_t _t0 = millis();
 ...
 */
// VENDORED: on timeout the BUSY waits below deselect (NSS high) and fail the
// transaction, which every caller already handles as an error — releasing the
// bus. See the PN5180_BUSY_TIMEOUT_MS note at the top of this file.
bool PN5180::transceiveCommand(uint8_t *sendBuffer, size_t sendBufferLen, uint8_t *recvBuffer, size_t recvBufferLen) {
  ...
  // Bounded replacement for `while (level != digitalRead(PN5180_BUSY));`. On
  // timeout: deselect and bail out of the whole transaction (return false).
  #define WAIT_BUSY(level) do { uint32_t _t0 = millis(); \
      while ((level) != digitalRead(PN5180_BUSY)) \
        if (millis() - _t0 > PN5180_BUSY_TIMEOUT_MS) { \
          digitalWrite(PN5180_NSS, HIGH); return false; } \
    } while (0)

  // 0.
  WAIT_BUSY(LOW);  // wait until busy is low
  // 1.
  ...
  // 3.
  WAIT_BUSY(HIGH);  // wait until BUSY is high
  // 4.
...
void PN5180::reset() {
  ...
  // bounded (vendored): reset() used to spin here forever if the chip never
  // answered, holding the bus and freezing displayTask. Time out and continue;
  // later commands will surface the error instead of the whole station hanging.
  { uint32_t _t0 = millis();
