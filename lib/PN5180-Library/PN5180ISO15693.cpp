...
ISO15693ErrorCode PN5180ISO15693::issueISO15693Command(uint8_t *cmd, uint8_t cmdLen, uint8_t **resultPtr) {
  ...
  // bounded (vendored): a tag whose SOF we detected but whose response never
  // completes would spin here forever holding the SPI bus. Time out and report no
  // card, which the callers handle — releasing the bus instead of freezing it.
  { uint32_t _t0 = millis();
    while (0 == (status & RX_IRQ_STAT)) {
      if (millis() - _t0 > PN5180_BUSY_TIMEOUT_MS) return EC_NO_CARD;
      delay(10);
      status = getIRQStatus();
    } }
