...
- **Rewriting Main destroys every field we do not model — 45 of the spec's 61 keys.** `optEncodeMain()` writes 16. A compliant vendor tag carrying GTIN, UUIDs, dates, density, drying and chamber temps, certifications or secondary colours loses all of them the moment we rewrite Main. Latent today because an adopted foreign tag's record is built *from* the tag, so the two agree and the reconcile loop never fires — but it goes live as soon as products become editable. Fix is verbatim passthrough of unrecognised key/value spans; see `docs/design/product-instance.md`.
