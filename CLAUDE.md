...
- **The portal is a TIMED window, and the first field trip was lost to it.** Carried to the lab, the station tried its bench credentials, failed, opened the portal, waited two minutes with nobody looking at it, closed it, and fell back to the SoftAP — which from the outside looks exactly like a device that never offered to be configured at all. Three things now say otherwise: the WiFi Setup screen shows a live `Closes in m:ss`, the no-WiFi idle screen shows `<addr>/reset` as the way back, and the Config page has a **Choose a WiFi network** button. `/reset` existed all along but was reachable only by typing the URL, so a station stranded on its own AP had no discoverable route onto a network.
