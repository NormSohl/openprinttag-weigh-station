...
- **`primary_color_lab` (key 59) is plumbed but never populated.** `OptMain` carries the three floats and a separate `has_lab` flag — separate because `L*` is legitimately 0 for a measured black and these structs are zero-initialised, so "all zeroes" must read as *unmeasured*. `optEncodeMain()` omits the key when unmeasured; the spec forbids approximating it from RGB, so there is no fallback to invent. If it is ever populated: OPT requires **D65/2°** (Nix apps often default to D50), and wound filament measures differently from a printed swatch — pick one.
- **Main rewrites preserve fields we do not model.** `optEncodeMain()` writes 20 of the spec's 61 Main keys, so a rewrite would otherwise *destroy* everything else a compliant vendor tag carried — GTIN, the four UUIDs, dates, density, drying and chamber temps, certifications, RAL, secondary colours — silently and irreversibly. `optDecode()` copies each unrecognised key/value pair verbatim into `OptMain.extra[]`, and `optEncodeMain()` splices them back before closing the map. They are already valid CBOR at map level, so nothing has to interpret them — which is the point. On overflow `extra_overflow` is set and `nfcTask` **refuses the rewrite**: losing an edit beats losing a vendor's data. `cborAppendRaw()` is the one place that touches tinycbor's encoder internals, and it is only safe because the map is *indefinite* length — `cbor_encoder_close_container()` then appends the break byte without checking `remaining`.
...
**The store layer is built and both write paths resolve a product.** `ProductRecord`, `StoreEv::Product`, the products index, compaction survival, the NVS product-id counter, `storeFindProduct()` / `storeAdoptProduct()`, and `DUMP prod`.
...
- **Reset (cabinet-installed):** the Config page's **Choose a WiFi network** button, or `POST` to `http://weighstation.local/reset` → device reboots into the captive portal. In SoftAP fallback the same button is at `http://192.168.4.1/`, which the idle screen prints and encodes as a QR.
- **The radio is 2.4 GHz only and PSK only.** An ESP32-S3 has no 5 GHz radio, and WiFiManager cannot do WPA2-Enterprise — so a 5 GHz-only SSID never appears in the portal's scan list, and an 802.1X network cannot be joined at all. Worth confirming about a venue's network *before* carrying the unit there.
- **`WiFiSetupMode` means "the captive portal is actually up" — nothing weaker.** It used to be set at the top of `syncTask`, before `autoConnect()`, so every ordinary boot flashed *"Join network: WeighStation-Setup"* for the half-second the join took, naming an AP that did not exist. That misdiagnosed a field trip: the flash read as a portal that had opened and timed out, when the station had joined a network immediately. **The idle screen is the reliable tell, and the two are easy to confuse in memory** — green *"Seattle Makers / Place spool to begin"* is `Idle`, i.e. joined; orange *"Weigh Station / Place spool to weigh / Join WiFi:"* is `IdleNoWiFi`, i.e. SoftAP fallback. `setState(Idle)` exists only in the joined branch.
- **The portal is a TIMED window, and it is easy to miss.**
...
- **The serial console lives in scaleTask.** ZERO, CAL, SEED, COMPACT, DUMP, STACK, APIKEY and TAGFORMAT are all dispatched from `handleSerialCommand()` there. `STACK` reports each task's high-water mark and the heap — run it *after* exercising the deep paths (place a spool, `DUMP`, `COMPACT`, load the web app), since the mark only records what has actually happened. Under ~512 B free is where to worry. The task used to `vTaskDelete` itself when the NAU7802 was missing, which silently removed every diagnostic command at the exact moment something was wrong. It now stays alive, keeps servicing commands, and retries the ADC every 5 s.
...
- **The Capability Container's MLEN is BYTES over 8, not the block count — and getting it wrong is invisible from here.** `optBuildBlankTag()` wrote `numBlocks` into CC byte 2, so an 80x4 tag declared 80*8 = 640 bytes of data area against a physical 320. Our own reader cannot notice: `nfcTask` dumps exactly `numBlocks` blocks and `optDecode()` parses the TLV straight out of that buffer, so nothing in this firmware ever consults MLEN. A **standards-compliant reader does**, and one that walks the declared area runs off the end of the tag — an ISO15693 tag answers out-of-range reads with an error, which presents as *"cannot read this tag"* on someone's phone while `DUMP TAG` is perfectly happy. Interop with other OPT-aware readers is the entire reason for being spec-compliant, so this is the shape of bug to watch for: correct by our own measurement, wrong by everyone else's. Integer division rounds down, which is the safe direction (a reader stops early rather than reading past the end); `tools/opt/optfuzz` now asserts MLEN against every geometry. Reference: `nfc_initialize.py` writes `args.size // 8`.
...
- **`pio run -t upload` uploads EVERY environment.** `platformio.ini` defines `weigh-station` and `diag`, and without `[platformio] default_envs` a bare `pio run -t upload` flashes both in order — so the diag sketch went on *after* the firmware and silently replaced it. The board came up printing `=== DIAG: PN5180 + TFT sharing one SPI host ===` and showing "shared bus diag" on the panel, which reads as the weigh station having lost its mind rather than as a different program. `default_envs = weigh-station` is now set; diag is opt-in with `-e diag`.
- **The Arduino build flashes `boot_app0.bin` at 0xE000 on EVERY upload, whether or not you use OTA — and `nvs` must stay below it.** `partitions.csv` had `nvs` at `0x9000` size `0x7000`, i.e. ending at `0x10000`, reclaiming the otadata hole that a single-app device does not need. But the toolchain writes there regardless of the partition table (`Flash will be erased from 0x0000e000 to 0x0000ffff` in every upload log), so the last two 4 KB pages of NVS were erased on every flash. NVS spreads entries across its pages, so the damage looked arbitrary and personal: the `scale` namespace, written once long ago, sat low and survived everything, while the spool-ID counter — rewritten at boot — sat high and vanished every time. The symptom was a counter that committed, read back correctly through a **fresh** handle, and was then flatly `ABSENT` on the next boot: nothing in the `Preferences` API can explain that, because nothing in it was wrong. `nvs` is `0x5000` now, ending at `0xE000`, which is exactly where the stock ESP-IDF layout puts it and why that layout leaves the hole. The 8 KB is left **unallocated** rather than declared as otadata, so nothing reads what the toolchain writes and the bootloader finds no otadata and boots `factory` unconditionally.
- **`Preferences::getString()` on a missing key logs an ESP_LOGE and then returns the default anyway.** `apiKeyBegin()` printed `nvs_get_str len fail: key NOT_FOUND` on every boot of a station with no API key — the normal case. Guard with `isKey()`. Same shape as the `LittleFS.exists()` trap below: ask whether it is there before reading, wherever the API lets you.
...
- **Arduino's `Print.h` defines `HEX`, `DEC`, `OCT` and `BIN` as bare macros** (`#define HEX 16`). A local named after a print base therefore expands to a numeric constant and fails to parse — `static const char* HEX = "0123456789ABCDEF";` in `urlEnc()` became `static const char* 16 = ...`. The error points at `Print.h`, not at your line, which is why it reads as a library problem. Prefix anything in that family: `kHexDigits`.
...
## Bench status & bring-up checklist
...
- **`PN5180::reset()` (and every other spin-wait in the library) used to be unbounded, holding the bus while it spun — a hung reader froze `displayTask`.** Fixed: the PN5180-Library is now **vendored in `lib/PN5180-Library`** with every wait bounded by `PN5180_BUSY_TIMEOUT_MS` (250 ms) — the five `BUSY`-line loops in `transceiveCommand()`, the `getIRQStatus()` waits in `reset()`/`setRF_on()`/`setRF_off()`, and the ISO15693 RX-response wait. On timeout a transaction fails cleanly (deselects NSS, returns an error the caller already handles) and releases the bus, instead of freezing. Normal ops complete in well under a millisecond, so 250 ms never false-triggers. **The library is a project-local copy now — `lib_deps` no longer pulls it from GitHub; edit `lib/PN5180-Library` for any further reader changes.**
