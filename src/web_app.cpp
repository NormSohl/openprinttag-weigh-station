    ...
    p += "<script>(function(){"
         ...
         "function poll(){"
           "fetch('/api/status').then(function(r){return r.json();}).then(function(d){"
             // The idle auto-off (see ERASE_MODE_IDLE_TIMEOUT_SEC) can clear
             // the mode with nobody touching this page at all -- reload
             // rather than keep showing a stale "ON" panel and Stop button
             // for a mode that isn't actually active any more.
             "if(!d.erase_mode){location.reload();return;}"
             "var s=d.state;"
...
static void handleApiEraseStart(AsyncWebServerRequest* req) {
    ...
    gEraseModeActivityMs = millis();   // starts the idle auto-off clock
    req->redirect("/erase");
}
...
static void handleReorder(AsyncWebServerRequest* req) {
    ...
    if (flagged) {
        ...
        String mailBody = "Below threshold at the weigh station:\n\n" + body
                         + "\nGenerated by the weigh station.\n";
        p += "<p><a style='display:inline-block;margin-top:4px;margin-right:8px;"
           ...
           + "&body=" + urlEnc(mailBody)
           + "'>E-mail this list</a>"
             // A mailto: link hands off to the OS's default mail app, which
             // isn't always set up (e.g. Outlook configured but not signed
             // in) -- found live. Copying plain text sidesteps that handoff
             // entirely: paste into whatever you actually use (webmail,
             // phone, Slack), no new device-side mail credentials needed.
             // The list lives in a textarea rather than a JS string literal
             // so esc()'s HTML-entity escaping (already needed for the
             // textarea itself) is the only escaping this needs. NOT hidden
             // with display:none: this station is deliberately HTTP-only
             // (no TLS -- see the API docs' Authentication section), and
             // navigator.clipboard requires a secure context, so it may not
             // exist here at all in some browsers. document.execCommand
             // ('copy') is the fallback that still works over plain HTTP,
             // but it operates on a selection, which needs the element to
             // actually be selectable -- off-screen positioning instead of
             // display:none, so it's still part of layout.
             "<button type='button' onclick=\"copyReorderList(this)\" "
             "style='padding:10px 18px;font-size:14px;border-radius:6px;"
             "cursor:pointer'>Copy list</button></p>"
             "<textarea id='reorder-body' readonly "
             "style='position:absolute;left:-9999px;top:-9999px'>"
           + esc(mailBody.c_str()) + "</textarea>"
             "<script>function copyReorderList(btn){"
               "var ta=document.getElementById('reorder-body');"
               "function done(ok){btn.textContent=ok?'Copied!':'Copy failed \\u2014 select manually';"
...
static void handleApiStatus(AsyncWebServerRequest* req) {
    ...
    // So /erase's own poll can notice the idle auto-off firing while the
    // page is open (nfc_task.cpp can silently clear this outside any web
    // request), not just show a stale "ON" after it's actually turned off.
    j += ",\"erase_mode\":"; j += gEraseModeActive ? "true" : "false";
